...
def bivariateFunction(alpha, beta, center, position):
    ...
    k = 10
    f = exp(-alpha*(position.x - center.x)/k **
            2 - beta*(position.y - center.y)/k**2)
    # print(f)
    return f
    ...
    def __init__(self):
        ...
        for i in range(0, 4):
            self.sprites.append(pg.image.load(
                f'models/Drone5/sprite_{i}.png').convert())
...
class FlowField():
    def __init__(self, resolution):
        ...
        self.field = [[vec(random.uniform(0, 1), random.uniform(0, 1)) for col in range(
            self.cols)] for row in range(self.rows)]  # create matrix
